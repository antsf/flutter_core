...
class RetryOptions {
  ...
  int calculateDelay(int attempt) {
    ...
    final exponentialDelay = baseDelayMs * pow(2, (attempt - 1).clamp(0, 30)); // Clamp to avoid overflow
    return (exponentialDelay > maxDelayMs ? maxDelayMs : exponentialDelay.toInt());
  }
}
...
class DioRetryInterceptor extends Interceptor {
  ...
  Future<void> onError(
    ...
    if (err.type == DioExceptionType.connectionTimeout ||
      ...
      shouldRetry = options.retryableStatusCodes.contains(err.response!.statusCode);
    } else {
    ...
    if (currentAttempt < options.maxAttempts && shouldRetry) {
      ...
      try {
        ...
        return handler.next(DioException(requestOptions: requestOptions, error: e, message: "Retry failed with non-Dio exception"));
      }
    } else {
